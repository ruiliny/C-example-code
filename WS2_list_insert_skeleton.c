/* In this exercise, you must insert a new node in the list after node n
...
void list_insert_after(list_t* list, node_t* n, void* d)
{
    ...
    // (0) Allocate memory for the new node called "new"
    
    assert(new != NULL);
    // (1) Place d at the node
    new->data = d;

    // (2) link the new node
    
    // (3) make sure the list pointers are up to date
